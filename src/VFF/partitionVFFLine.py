...
class partitionVFFLine(Many):
    def __init__(self, cx, cy, dx, dy, vff):
        self.cx = cx
        self.cy = cy
        self.vff = vff
        self.dx = dx
    ...
    def many(self):
        ...
        sample = PolyLine(recentered.sampl(Linspace(100))).many()
        samples = list(zip(Linspace(100).values(),sample))
